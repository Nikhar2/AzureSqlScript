...
CREATE PROCEDURE [dbo].[RemoveAmazonSchedules] AS
                        BEGIN
                        declare amz_tns CURSOR for select job_name from qrtz_triggers where job_group in ('LoadOrdersFromAmazon', 'LoadTrackingNumbersToAmazon', 'CheckAmazonForPendingOrders')
                        open amz_tns
                        ​
			    ...
			    EXEC sp_execute_remote @data_source_name  = N'ElasticDBQuartz',
		            @statement = N'exec deletejob @jobid'
                            fetch next from amz_tns into @jobid;
                                    end;
                        ​
                             close amz_tns
